...
Update members
set member_address='85/3,Itkhola'
...
BEGIN
    ...
    -- Update book status directly using subquery
    UPDATE books
    SET status = 'yes'
    WHERE isbn = (
    ...
    -- Display thank-you message with inline book name
...
b.branch_id,
b.manager_id,
count(ist.issued_book_isbn)as numbers_of_books_issued,
count(rs.return_id)as numbers_of_books_returned,
...
as
select*from members
where member_id in (Select distinct issued_member_id from issued_table where issued_date>= current_date - interval '2 month');


--Find Employees with the Most Book Issues Processed.Write a query to find the top 3 employees who have processed the most book issues. Display the employee name, number of books processed, and their branch.
...
Create or replace procedure issue_table(p_issued_id varchar(20),p_issued_member_id varchar(10), p_issued_book_isbn VARCHAR(25),p_issued_emp_id VARCHAR(10))
language plpgsql
as $$
Begin
if exists ( select 1 from books where isbn=p_issued_book_isbn and status= 'yes' )
Then
...
CALL issue_table('IS155', 'C108', '978-0-553-29698-2', 'E104');
CALL issue_table('IS156', 'C108', '978-0-375-41398-8', 'E104');
